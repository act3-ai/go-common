...
func FlagUsages(f *pflag.FlagSet, opts UsageFormatOptions) string {
	...
	for _, line := range lines {
		...
		if found {
			...
			// maxlen + 3 comes from:
			//  + 1 for spacing
			//  + 1 for the \x00 which is ignored by ansi.StringWidth
			//  + 1 for the (deliberate) off-by-one in maxlen-ansi.StringWidth(before)
			//
			// Original comment from pflag package:
