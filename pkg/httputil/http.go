...
func (fn RootHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if !errors.As(err, &clientError) {
		...
		if err := WriteJSON(w, map[string]any{"instance": uid, "statusCode": http.StatusInternalServerError}); err != nil {
			log.Error(err, "Failed to write error body")
		}
		return
	}

	// It is a ClientError
	log.V(1).Info("ClientError", append([]any{"error", clientError.Error()}, clientError.ErrorArgs()...)...)

	// Provide the error to the client
	body, err := clientError.ResponseBody()
	if err != nil {
		log.Error(err, "Failed to get the response body")
		w.WriteHeader(http.StatusInternalServerError)
		return
	}
